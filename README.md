...
ビルドは不要ですが、ブラウザのセキュリティ制約のため **ローカルサーバー経由** で開いてください。
...
> `npx serve` や VS Code の Live Server 拡張など、お好みの静的サーバーでも構いません。
...
- **グラフが真っ白で何も表示されない**
  `file://` で直接開いている可能性があります。ローカルサーバー経由で開いてください。
  また、Chart.js の CDN 読み込みにはインターネット接続が必要です。
