...
└── samples/
    ...
    ├── pie.html / pie.js         # ドーナツ: 特定年の世界 GDP に占める国別シェア
...
世界全体 (`WLD`) の値も一緒に取得し、選択国以外を「その他」として残差で加えます。
これにより全要素の合計が世界総計と一致し、「世界に占めるシェア」という意味のある構成比になります
(円・ドーナツグラフは、合計が意味のある全体になるデータで使うのが原則です)。
WLD のデータがある最新年から `CONFIG.interval` 年間隔で遡り、新しい年から順に
複数のドーナツグラフを 3 列のグリッドで描画します。凡例はページ上部の共有のものを使い、
国ごとの色は全グラフで統一しています。
ツールチップのコールバックで、値と世界総計に占める割合 (%) を併記しています。
